...
class MainActivity : AppCompatActivity(), DeleteAllDialog.DialogListener {
	...
	private val getExport =
		registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
			if (result.resultCode == RESULT_OK) result.data?.data?.also(this::exportData)
		}
	...
	private val settingsResult =
		registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
			if (result.resultCode == RESULT_OK) history.limit = settings(this).undoLength
		}
	
	private val editResult =
		registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
			if (result.resultCode == RESULT_OK) (result.data?.getSerializableExtra(CATEGORY) as? Category)?.also { cat ->
				data(this).categories[cat.id] = cat
				val categories = findViewById<LinearLayout>(R.id.categories)
				(((categories.children.firstOrNull { cat.id == it.tag } as? LinearLayout)?.getChildAt(
					CATEGORY_HEADER_INDEX
				) as? LinearLayout)?.getChildAt(CATEGORY_TITLE_INDEX) as? TextView)?.text = cat.name
			}
		}
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		this.listsAdapter =
			ArrayAdapter(this, android.R.layout.simple_selectable_list_item, ArrayList())
		spinnerList.adapter = this.listsAdapter
		spinnerList.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
			override fun onItemSelected(
				adapter: AdapterView<*>?, view: View?, index: Int, p3: Long
			) {
	...
	override fun onRequestPermissionsResult(
		...
		when (requestCode) {
			REQUEST_PERMISSION_CODE -> {
				if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) openFilePicker()
			}
		}
	}
	...
	private fun importData(uri: Uri) {
		...
		import(this, uri)?.also { importData ->
			val data = data(this)
			val targetList =
				data.lists.getOrPut(importData.listName ?: data.currentList, ::CheckList)
			
			fun skipTask(catId: Long, iTask: String): Boolean {
				return settings.skipExistingTasks && targetList.tasks[catId]?.contains(iTask) ?: false
			}
	...
	private fun exportData(uri: Uri) {
		...
		list.tasks.forEach { catId, tasks ->
			if (tasks.isNotEmpty()) data.categories[catId]?.also { category ->
	...
	private fun changeTask(action: ChangeTask) {
		...
		if (action.catNew == action.catOld) {
			if (action.indOld != action.indNew) {
				Log.e(
					CHANGE_TASK,
					"Expected indNew (${action.indNew}) to == indOld (${action.indOld})"
				)
				return
			}
			...
			if (action.indOld !in IntRange(0, tasksOld.childCount)) {
				Log.e(
					CHANGE_TASK, "indOld (${action.indOld} not in range [0, ${tasksOld.childCount})"
				)
				return
			}
			if (action.indNew != -1 && action.indNew !in IntRange(0, tasksNew.childCount + 1)) {
				Log.e(
					CHANGE_TASK,
					"Expected indNew to be -1 or be in range [0, ${tasksNew.childCount + 1}) but was ${action.indNew}"
				)
				return
			}
	...
	private fun getOrCreateCategory(categoryId: Long): RecyclerView {
		if (categoryId == UNCATEGORIZED) return findViewById(R.id.taskList)
